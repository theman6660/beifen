...
function sourceIsOfficial(item) {
  ...
  return OFFICIAL_SOURCE_NAMES.has(source) || /\bofficial\b/i.test(source);
}
...
function validateChronicleCandidate(candidate, evidenceItems) {
  ...
  const distinctSources = new Set(sources.map(item => item.source).filter(Boolean));

  if (!eventDate) return { pass: false, reason: '事件日期无效' };
  if (event.length < 16) return { pass: false, reason: '事件描述过短' };
  if (Array.from(event).length > 180) return { pass: false, reason: '事件描述过长，不符合编年史简洁风格' };
  ...
  if ((distinctLinks.size < 2 || distinctSources.size < 2) && !sources.some(sourceIsOfficial)) {
