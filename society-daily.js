...
2. 使用 Markdown 二级标题组织结构。板块顺序固定，只输出有足够素材支撑的板块（至少2条关联新闻），无素材则跳过：
   ## 今日关键词（必有）
   ## 时代切片（必有）
   ## 关系透视
   ## 心理地貌
   ...
   ## 时代回响（必有）
3. 各板块说明：
   - **今日关键词**：3-5个词或短语，一行一个，格式：\`- **关键词** — 一句话注解\`。勾勒今日情绪地图，让读者一眼看到今天的主题脉络。不展开。
   - **时代切片**：标题必须带副标题，格式 \`## 时代切片：副标题\`。选择1-2条新闻，重点不是事件本身，而是它揭示了什么样的集体心理。至少有一个 **加粗的关键洞察句**。
   - **关系透视**：标题必须带副标题。社会关系的变化——亲密关系的形态、代际之间的张力、个体与群体的博弈、信任/疏离模式。
   - **心理地貌**：标题必须带副标题。这个时代人们普遍的心理状态——焦虑的来源、逃避的方式、自我认同的困境、意义感的缺失或重建。
   - **时代精神**：标题必须带副标题。正在形成或瓦解的集体信念、价值排序的位移、"什么是好的生活"的定义正在如何被改写。
   - **一面镜子**：标题必须带副标题。一个问题或观察，让读者停下来想一想自己：你是否也在这个模式里？你真正想要的是什么？
   - **时代回响**：2-4句话收束全文。不需要新素材，而是把前文的线索编织起来，给出一个有力的收尾观察。像一个朋友聊完天后最后的沉默片刻。
4. 对每条分析，请遵循深度分析结构：
   （1）事件概述
...
9. 总字数控制在2000-3500字。每个输出的板块不少于400字（今日关键词除外，50-80字即可）。
...
检查标准：
1. 是否有"今日关键词"板块？是否包含3-5个关键词？
2. 各板块标题是否带有副标题（格式：## 板块名：副标题）？
3. 是否有"时代回响"收尾板块？
