...
// ============ 配置 ============
const MAX_REPORT_TOKENS = Number.parseInt(process.env.MAX_TOKENS || '8000', 10) || 8000;
...
async function generateReport(newsItems, retryFeedback = '') {
  ...
  let prompt = `你是一位兼具社会学家和精神分析师视角的信息观察者。你的任务是从近期新闻中，广泛捕捉这个时代的集体心理、社会关系和精神世界。

是透过事件看到人心的人。

核心目标：优先保证信息广度和信号覆盖。不要把日报写成一篇长论文；用短观察连接多个材料，让读者看到今天社会心理和生活现场的整体轮廓。

写作策略：先在脑中选出3-5条最重要的社会心理信号。它们可以在"今日信号"中一行出现，但后续板块不要再换句话复述同一事实；后续板块优先写新的关系结构、心理机制、生活场景或时代判断。若同一事件确实需要跨板块引用，只能增加新的角度、对照或后续影响，不要重复"发生了什么"。
...
3. 各板块说明：
   ...
   - **今日信号**：选择5-10条值得知道的材料。每条格式：\`- **事件/现象**：一句话概述 + 一句话说明它折射的心理或社会关系。\` 这里只做事实入口，后续板块不要把同一事实重新讲一遍。
   ...
   - **时代回响**：3-6句话收束全文，只写跨材料的综合判断或待观察变量，不要把前文新闻再复述一遍。
4. 对每条分析，请遵循轻分析结构：
   （1）发生了什么
   （2）它折射了什么心理/社会关系
   （3）为什么值得继续看
...
8. 去重要求：
   - 同一新闻事实最多在"今日信号"和一个正文分析板块中出现；不要在"关系透视""心理地貌""生活现场""时代精神""时代回响"之间反复出现。
   - 同一主题可以被多次讨论，但每次必须提供新的心理机制、关系结构、生活场景或时代判断。
   - 如果某个板块没有足够未重复材料，可以缩短或跳过，不要为了凑板块复述。
9. 严格排除：不要写政治、国际关系、军事、外交、政党相关内容。只关注人的心理和社会关系。
10. 长度不作为硬目标。素材多可以写长，素材少就保持简洁；宁可覆盖更多社会心理信号，也不要为了凑字扩写。
11. 不要编造来源；不要输出参考来源列表，脚本会自动追加
12. 直接输出文章内容，不要加markdown代码块标记`;

  if (retryFeedback) {
    prompt += `\n\n【重要提示！上一次生成未通过质检审查，原因如下，请在本次生成中务必严格规避并针对性修正】：\n${retryFeedback}`;
  }
...
7. 今日关键词与今日信号作为概览可以提及核心事实；但正文各分析板块（"关系透视""心理地貌""生活现场""时代精神""时代回响"）之间不得相互换句话反复重述同一事实。如果正文分析板块之间有明显重复，应 FAIL。
8. 字数长短、分析深度不作为失败理由；只在结构明显缺失、信息覆盖明显过窄、重复明显或明显跑题时 FAIL。
...
async function generateWithRetry(promptNewsItems, dateStrCN, maxRetries = 3) {
...
function parseArgs() {
  ...
  return { force, daysAgo };
}

// ============ 主流程 ============
async function main() {
  const { force, daysAgo } = parseArgs();
